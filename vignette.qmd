...
#     )
# 
# saveRDS(pam_clustered, file = "vignette_files/model-results/PAM_model.rds")

# Reading in the PAM model from files since the model takes a long time to run
pam_clustered <- readRDS("vignette_files/model-results/PAM_model.rds")
```
...
fviz_cluster(list(clusters = pers_combined[,31], data = pers_combined[,2:30]), geom = "point", ellipse = TRUE, main = "DBScan Clustering w/o noise values") + theme_bw()
```
...
```{r}
pers_summary_transpose <- pers_combined %>% group_by(cluster) %>% summarise(across(where(is.numeric), mean, .names = "mean_{.col}"), .groups = "drop") %>% t()
colnames(pers_summary_transpose) <- pers_summary_transpose[1,]
pers_summary_transpose <- pers_summary_transpose[-1,]
pers_table <- pers_summary_transpose %>% kbl() %>% kable_material("striped")

htmltools::div(
style = "height:250px; overflow-y: scroll; border:1px solid #ccc; padding:5px;",
htmltools::HTML(pers_table, format = "html", caption = "The mean of each variable across the three clusters"))
```
...
```{r choose-k-elbow, warning=FALSE}
...
```{r choose-k-silhouette, warning=FALSE}
...
```{r k-means-result-visualization}
...
# Conclusion
Across all four clustering methods, including PAM, COP-KMeans, DBSCAN, and K-means, our analysis reveals a consistent structure in the *Personality Synthetic Dataset*. Despite using different definitions of similarity, each method identifies clear separation along the dataset's dominant personality dimension, mainly along the introversion–extroversion axis. This phenomenon across models demonstrate that the dataset contains meaningful psychological structure.

The distance-based methods, such as PAM, COP-KMeans, and K-means, perform well because the clusters in this dataset are compact and roughly spherical. PAM and COP-KMeans both concluded three groups corresponding to introverts, extroverts, and an intermediate ambivert-like cluster. Additionally, K-means, optimized for simple geometric separation, identifies the strongest two-group structure, which is the introverts-extroverts structure, but does not consistently detect the subtler third cluster, which is the ambivert cluster.

By contrast, DBSCAN uses a density-based approach, which is advantageous for discovering irregular cluster shapes and identifying noise. However, in this high-dimensional setting, density estimation becomes unstable, causing DBSCAN to flag many points as noise. Even so, the major clusters it identifies remain consistent with the distance-based methods.

Conclusively, the comparison reveals that while all methods capture the dominant personality dimension, their effectiveness depends on the assumptions they impose. K-means is efficient and stable for large numeric datasets; PAM offers robustness and interpretability; COP-KMeans excels when slight supervision is available; and DBSCAN is best suited for non-spherical clusters and outlier detection. These results demonstrate how clustering algorithms with different philosophies can provide complementary perspectives while ultimately converging on the same core structure within the data.

Furthermore, it is important to note that this was a synthetically generated dataset. Real world dataset might have problems dealing with correaltion, missing data, class imbalance (although all four clustering methods perform relatively well with class imbalance datasets as well). If you have highly correlated dataset you might expect a wider spread in the clusters. Our dataset had a lot of correlation too (see the heatmap in the [EDA section](https://github.com/pstat197/vignette-Supervised-Unsupervised-Clustering/blob/main/Scripts/drafts/ExploratoryAnalysis.Rmd)), however, the methods we deployed could correctly identify the clusters. With a lot of correlation you might expect wider clusters since the model will be confusing the correlation between variables more than they are actually used for predicting the personaility type. This exact reason is why we see wider clusters when we have correlation. 

If the boundaries are getting blurry when you have correlation, please refer to other resources to deal with your correlation accordingly. Some suggestions from us is deploying a variable selection method before continuing with clustering. Some common variable selection methods widely used are Lasso (if the data is linear), Randomized Lasso, Elastic Net, Stepwise selection, Graphical Modelling, Principle Component Analysis (PCA was done in the code for visualizing some of the clustering methods we used as well), and even some Tree-Based importance methods. 
